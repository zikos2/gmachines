(function($) {
	...
	$(function() {
		...
		// income-chart start
		if ($('#sumPerSalleChart').length) {
		(function run(){
		var sumPerSalleChartCanvas = $("#sumPerSalleChart").get(0).getContext("2d");

			const labels = [];
			const sumData = [];


			fetch("/GMachineAajax/MachineController?op=pricePerSalle")
				.then(res => res.json())
				.then(data => {
					data.forEach(item => labels.push(item.code));
					data.forEach(item => sumData.push(item.prix));
					const optionsTwo =  {
				  type: 'bar',
				  data: {
				    labels,
			...
			let sumPerSalleChart = new Chart(sumPerSalleChartCanvas, optionsTwo);
		...
		// Machines count start
		
		
		if ($('#machinesCount').length) {
		(function run(){
		var machinesCountCanvas = $("#machinesCount").get(0).getContext("2d");

			const labels = [];
			const count = [];


			fetch("/GMachineAajax/MachineController?op=machinesCount")
				.then(res => res.json())
				.then(data => {
					data.forEach(item => labels.push(item.salleCode));
					data.forEach(item => count.push(item.count));
					const optionsTwo =  {
				  type: 'bar',
				  data: {
				    labels,
			...
			let machinesCountChart = new Chart(machinesCountCanvas, optionsTwo);
		...
		if ($("#sales-chart-c").length) {
			...
			fetch("/GMachineAajax/MachineController?op=statusCount")
			.then(response=>response.json())
			.then((data)=>{
			console.log(data);
			const labels = data.map(item=>item.status)
			const count = data.map(item=>item.count)
			$("#machinesOncount").html(count[1]);
			$("#machinesOffcount").html(count[0]);
	...
	logout.addEventListener("click",()=>{
		fetch("/GMachineAajax/User?op=logout")
		.then(response => window.location.replace("http://localhost:8082/GMachineAajax/pages/login.jsp"))
		.catch(err=>console.log(err))
	})
})(jQuery);